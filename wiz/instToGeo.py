				...
				if pid not in pids[instList.index(inst)]:
					...
					if not objExists( instGrp ):
						...
						if dupOrInst == 2:
							...
							dup = duplicate( instGeo[int(idx)], name = instGeo[int(idx)] + '_instToGeo_instObj' )[0]
						parent( dup, instGrp )
					...
					if dupOrInst == 2:
						...
						dupGrp = duplicate( instGrp, name = dupName + '_grp', inputConnections = True, instanceLeaf = False )[0]
						dup = rename( listRelatives( dupGrp, children = True, fullPath = True )[0], dupName )
						if dupOrInst == 1:
